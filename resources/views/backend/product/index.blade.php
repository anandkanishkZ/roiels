    ...
    <section class="content">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">{{$panel}} {{$action}} <a href="{{route($base_route . '.create')}}" class="btn btn-info">  <i class="fa fa-plus"></i> Create {{$panel}}</a> <a href="{{route('backend.order.index')}}" class="btn btn-success">  <i class="fa fa-shopping-basket"></i> View Order</a></h3>
            </div>
            <div class="card-body">
                        ...
                        <tr>
                            ...
                            <td>
                                @if($row->image == '')
                                    ...
                                    @else
                                    <img src="{{asset($image_path . '/' . $row->image)}}"  width="100" height="100" alt="{{$row->title}}"> </td>
                                    @endif
                            <td>
                                @if($row->status == 1)
